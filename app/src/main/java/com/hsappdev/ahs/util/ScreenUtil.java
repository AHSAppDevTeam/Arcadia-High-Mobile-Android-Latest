...
public class ScreenUtil {
    ...
    /**
     * Formats titles so that the first letter of every word is capitalized
     */
    public static String capitalizeFully(String text){
        if(text == null) return null;
        StringBuffer textBuffer = new StringBuffer(text.toLowerCase(Locale.ROOT).trim());
        int index = -1;
        do{
            if(index++ < textBuffer.length()){ // note addition operation
                textBuffer.setCharAt(index, Character.toUpperCase(textBuffer.charAt(index)));
            }
        } while ((index = textBuffer.indexOf(" ")) != -1);
